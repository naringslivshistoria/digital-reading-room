...
export const indexLevel = async (level: number, skip: number, maxResults: number) => {
  const url = `${config.comprimaUrl}/indexLevel?level=${level}&skip=${skip}&maxResults=${maxResults}`;
